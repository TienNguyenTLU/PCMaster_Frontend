    ...
    >
      ...
      <div className="flex flex-col flex-1 p-4 gap-2.5 z-0 justify-between">
            ...
            <div className="flex flex-col gap-1.5 bg-[#f8fafc] rounded-[12px] p-2.5 border border-slate-100 mt-1 relative z-20 text-left">
              ...
              <div className="flex flex-col gap-0.5">
                {product.pcComponents.slice(0, 3).map(comp => (
                  <div key={comp.componentProductId} className="flex items-center gap-1 text-[11px] text-[#475569] font-medium truncate">
                    <span className="text-[#0058be] font-bold text-[8px]">•</span>
                    <span className="truncate">{comp.componentProductName}</span>
                  </div>
                ))}
                {product.pcComponents.length > 3 && (
                  <p className="text-[9px] font-bold text-[#0058be] mt-0.5">Và {product.pcComponents.length - 3} linh kiện khác...</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-[#f1f5f9] relative z-20 mt-2">
          <div>
            <p className="text-[9px] font-bold text-[#94a3b8] uppercase tracking-[0.5px]">Giá bán lẻ</p>
            <p className={`text-[16px] font-bold ${product.stock === 0 ? 'text-[#94a3b8]' : 'text-[#0058be]'}`} style={{ fontFamily: 'Inter, sans-serif' }}>
