...
import { X, Search, Plus, Minus, Trash2, Loader2, Package, Check } from 'lucide-react';
import { adminAPI, orderAPI, Product, IssueSlipResponse, OrderResponse } from '@/lib/api';
...
export default function CreateIssueSlipModal({ onClose, onSuccess }: CreateIssueSlipModalProps) {
  const [reason, setReason] = useState<'RETAIL_SALE' | 'PROVIDER_RETURN' | 'PC_ASSEMBLY'>('RETAIL_SALE');
  ...
  const handleSelectItem = (product: Product) => {
    ...
    setSelectedItems(prev => {
      ...
      if (existing) {
        toast.error('Sản phẩm đã được chọn. Vui lòng điều chỉnh số lượng ở danh sách phía dưới.');
        return prev;
      }
  ...
  const handleSubmit = async () => {
    ...
    for (const item of selectedItems) {
      if (item.quantity > item.product.stock) {
        toast.error(`Sản phẩm "${item.product.name}" vượt quá số lượng tồn kho khả dụng!`);
        return;
      }
    }
    ...
    try {
      ...
      toast.error(error?.response?.data?.message ?? 'Đã có lỗi xảy ra khi tạo phiếu xuất kho');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0] bg-[#f8fafc]">
          <div>
            <h3 className="font-semibold text-[18px] text-[#0f172a]">Tạo phiếu xuất kho trực tiếp</h3>
            <p className="text-[#64748b] text-[13px] mt-0.5">Xuất kho nhanh và tự động cập nhật số lượng tồn kho</p>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            <h4 className="text-[12px] font-bold text-[#64748b] uppercase tracking-[0.5px]">Lý do xuất kho</h4>
            <div className="flex flex-col gap-2.5">
              {EXPORT_REASONS.map(r => {
                ...
                return (
                  ...
                  >
                    <div className={`mt-0.5 size-4 rounded-full border flex items-center justify-center shrink-0 mr-3 ${
                      isSelected ? 'border-[#0058be] bg-[#0058be]' : 'border-[#cbd5e1] bg-white'
                    }`}>
                      {isSelected && <div className="size-1.5 rounded-full bg-white" />}
                    </div>
                    <div>
                      <p className={`text-[14px] font-semibold ${isSelected ? 'text-[#0058be]' : 'text-[#0f172a]'}`}>{r.label}</p>
                      <p className="text-[12px] text-[#64748b] mt-0.5">{r.description}</p>
                    </div>
                  </button>
                );
              })}
          ...
          {reason === 'RETAIL_SALE' && (
            <div className="space-y-3 bg-blue-50/10 border border-[#0058be]/10 rounded-[12px] p-4">
              <h4 className="text-[12px] font-bold text-[#64748b] uppercase tracking-[0.5px] flex items-center gap-2">
                ...
                {loadingOrders && <Loader2 className="size-3.5 text-[#0058be] animate-spin" />}
              </h4>
              <select
                  ...
                  if (val) {
                    ...
                    if (ord) {
                      const itemsList = ord.items.map(item => {
                        ...
                        return {
                          product: prod || ({ id: item.productId, name: `Linh kiện mã #${item.productId}`, stock: item.quantity, price: item.sellingPrice } as Product),
                          quantity: item.quantity
                        };
                      });
              ...
              >
                ...
                {orders.map(o => (
                  <option key={o.id} value={o.id}>
                    Đơn #{String(o.id).padStart(5, '0')} - {o.recipientName || 'Khách lẻ'} ({new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(o.totalAmount)})
                  </option>
                ))}
              </select>
              {orders.length === 0 && !loadingOrders && (
                <p className="text-[12px] text-amber-600 font-medium">Hiện tại không có đơn hàng nào chờ xuất kho.</p>
              )}
            </div>
          )}
          ...
          {reason !== 'RETAIL_SALE' && (
            <div className="space-y-3">
              <h4 className="text-[12px] font-bold text-[#64748b] uppercase tracking-[0.5px]">Chọn sản phẩm xuất</h4>
                          ...
                          <div className="flex items-center gap-3">
                            {p.thumbnailUrl ? (
                              p.thumbnailUrl.startsWith('http://localhost') ? (
                                <img src={p.thumbnailUrl} alt={p.name} className="h-8 w-8 object-contain shrink-0" />
                              ) : (
                                <CldImage src={p.thumbnailUrl} alt={p.name} width={32} height={32} className="h-8 w-8 object-contain shrink-0" />
                              )
                            ) : (
                            ...
                            <div>
                              <p className="text-[13px] font-semibold text-[#0f172a] line-clamp-1">{p.name}</p>
                              <p className="text-[11px] text-[#94a3b8] mt-0.5">Mã SP: #{p.id}</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className={`text-[12px] font-semibold ${p.stock > 0 ? 'text-[#334155]' : 'text-red-500'}`}>
                              Tồn: {p.stock}
                            </span>
                          </div>
                        </button>
            ...
            {selectedItems.length === 0 ? (
              ...
              <div className="border border-[#e2e8f0] rounded-[12px] overflow-hidden divide-y divide-[#e2e8f0]">
                {selectedItems.map(item => (
                  <div key={item.product.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white hover:bg-[#f8fafc]/30 transition-colors">
                    {/* Product Details */}
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      {item.product.thumbnailUrl ? (
                        item.product.thumbnailUrl.startsWith('http://localhost') ? (
                          <img src={item.product.thumbnailUrl} alt={item.product.name} className="h-10 w-10 object-contain shrink-0" />
                        ) : (
                          <CldImage src={item.product.thumbnailUrl} alt={item.product.name} width={40} height={40} className="h-10 w-10 object-contain shrink-0" />
                        )
                      ) : (
                      ...
                      <div className="min-w-0">
                        <p className="text-[13.5px] font-semibold text-[#0f172a] truncate">{item.product.name}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[11px] text-[#94a3b8]">Mã: #{item.product.id}</span>
                          <span className="text-[11px] text-[#94a3b8]">•</span>
                          <span className="text-[11px] font-semibold text-emerald-700">Tồn kho khả dụng: {item.product.stock}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto shrink-0 border-t sm:border-0 pt-2.5 sm:pt-0">
                      <div className="flex items-center gap-1.5">
                        {reason !== 'RETAIL_SALE' ? (
                          ...
                          <div className="text-right">
                            <span className="text-[11px] text-[#94a3b8] font-bold block">Số lượng xuất</span>
                            <span className="text-[14px] font-bold text-[#0f172a]">{item.quantity} cái</span>
                          </div>
                        )}
                      </div>
