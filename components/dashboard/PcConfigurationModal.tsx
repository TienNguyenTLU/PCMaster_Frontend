...
import {
  PC_COMPONENT_CATEGORY_LABELS,
  PC_COMPONENT_CATEGORY_ICONS,
...
export default function PcConfigurationModal({
  ...
  const getComponentCategory = (p: Product): ComponentCategory | "other" => {
    ...
    if (
      ...
      name.includes("cpu") ||
      name.includes("vi xu ly")
    )
      return "cpu";
    if (
      slug.includes("mainboard") ||
      slug.includes("motherboard") ||
      slug.includes("bo-mach") ||
      name.includes("main") ||
      name.includes("bo mach")
    )
      return "mainboard";
    if (
      slug.includes("vga") ||
      ...
      slug.includes("card") ||
      name.includes("vga") ||
      name.includes("card do hoa") ||
      name.includes("card man hinh")
    )
      return "vga";
    if (
      slug.includes("ram") ||
    ...
    if (
      ...
      slug.includes("vomay") ||
      name.includes("case") ||
      name.includes("vo may")
    )
      return "case";
    if (
      slug.includes("cooler") ||
      slug.includes("tannhiet") ||
      name.includes("tan nhiet") ||
      name.includes("cool")
    )
      return "cooler";
    if (
  ...
  useEffect(() => {
    ...
    if (editingProduct) {
      ...
      setBrandId(
        String(editingProduct.brandId || editingProduct.brand?.id || ""),
      );
      ...
      const newCompQtys: ComponentQuantities = {
        mainboard: 1,
        cpu: 1,
        vga: 1,
        ram: 1,
  ...
  const pcSystemCategoryId =
    categories.find((c) => c.slug === "pc-system")?.id || "";

  // Create products map for quick lookup
  const productsMap = productList.reduce(
    (acc, p) => {
      acc[Number(p.id)] = p;
      return acc;
    },
  ...
  const getFilteredOptions = (cat: ComponentCategory): Product[] => {
    const rawList = productList.filter(
      (p) =>
    ...
    if (cat === "mainboard") {
      ...
      if (cpuSocket) {
        filtered = filtered.filter(
          (p) => getProductSpecs(p).socket === cpuSocket,
        );
      }
      if (ramType) {
        filtered = filtered.filter(
          (p) => getProductSpecs(p).ram_type === ramType,
        );
      }
      if (selectedCase) {
    ...
    if (cat === "cpu") {
      ...
      if (mainboardSocket) {
        filtered = filtered.filter(
          (p) => getProductSpecs(p).socket === mainboardSocket,
        );
      }
      if (selectedCooler) {
        filtered = filtered.filter((p) => {
    ...
    if (cat === "ram") {
      if (mainboardRamType) {
        return rawList.filter(
          (p) => getProductSpecs(p).ram_type === mainboardRamType,
        );
      }
    }
  ...
  const componentBuildLimits = activeSelections.map((item) => {
    ...
    if (!prod)
      return { id: item.id, name: "Chưa chọn", limit: 999999, stock: 0 };
    const limit =
      item.quantity > 0 ? Math.floor(prod.stock / item.quantity) : 999999;
  ...
  const bottleneck = componentBuildLimits.find(
    (c) => c.limit === maxBuildableQuantity,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pcSystemCategoryId) {
      setSubmitError(
        "Không tìm thấy danh mục PC_SYSTEM trong hệ thống. Vui lòng liên hệ nhà phát triển.",
      );
      return;
    }
    ...
    try {
      ...
      const msg =
        ...
        "Có lỗi xảy ra khi tạo cấu hình PC.";
      setSubmitError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-[16px] shadow-2xl w-full max-w-4xl max-h-[92vh] overflow-y-auto mx-4 flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0] sticky top-0 bg-white rounded-t-[16px] z-10">
          <div>
            <h3 className="text-[#0f172a] text-[18px] font-semibold flex items-center gap-2">
              ...
              {isEditing
                ? "Cập nhật cấu hình PC lắp sẵn"
                : "Lắp ráp sản phẩm PC mới"}
            </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
                    ...
                    className={`border rounded-[8px] px-3 py-2 text-[14px] focus:outline-none transition-all shadow-sm ${
                      isEditing
                        ...
                        : errors.stock
                          ...
                          : "bg-white border-[#e2e8f0] focus:border-[#0058be]"
                    }`}
                  />
                  {!isEditing && (
                ...
                {thumbnailPreview ? (
                  <div className="flex flex-col items-center gap-2 w-full h-full justify-center relative">
                    <img
                      ...
                      alt="preview"
                      className="h-28 object-contain rounded-[8px] border border-[#e2e8f0] bg-white p-1"
                    />
                    <p className="text-[11px] text-[#94a3b8] truncate max-w-[200px]">
                      {thumbnailFile?.name || "Ảnh đã chọn"}
                    </p>
                  </div>
                ) : (
                  <>
          ...
          <div className="flex flex-col gap-2 bg-slate-50/50 p-4 rounded-[12px] border border-slate-100">
            <label className="text-[13px] font-semibold text-[#374151]">
              Nhu cầu sử dụng (Chọn nhiều nhu cầu phù hợp)
            </label>
          ...
          <div className="flex flex-col gap-4 border-t border-[#e2e8f0] pt-4">
            ...
            {!isEditing &&
              ...
              bottleneck.id > 0 && (
                <div className="flex items-center gap-2 bg-[#fff7ed] border border-[#ffedd5] text-[#c2410c] px-4 py-2.5 rounded-[8px] text-[13px] shadow-sm animate-pulse">
                  ...
                  <span>
                    Số lượng lắp ráp tối đa bị giới hạn bởi:{" "}
                    <strong>{bottleneck.name}</strong> (Còn{" "}
                    <strong>{bottleneck.stock}</strong> cái trong kho, mỗi bộ
                    cần 1)
                  </span>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {CATEGORY_KEYS.map((cat) => {
                const Icon = PC_COMPONENT_CATEGORY_ICONS[cat];
                const label = PC_COMPONENT_CATEGORY_LABELS[cat];
                const selectedId = compIds[cat];
                const selectedProd = productsMap[selectedId];
                const options = getFilteredOptions(cat);
                const isFiltered =
                  (cat === "mainboard" && (cpuSocket || ramType)) ||
                ...
                if (isFiltered) {
                  if (cat === "mainboard") {
                    if (cpuSocket && ramType)
                      compatTag = `Đã lọc: Socket ${cpuSocket} & ${ramType}`;
                    else if (cpuSocket)
                      compatTag = `Đã lọc: Socket ${cpuSocket}`;
                ...
                return (
                  <ComponentCard
                    ...
                    qty={compQtys[cat]}
                    compatTag={compatTag}
                    onComponentChange={(productId) =>
                      handleComponentChange(cat, productId)
                    }
