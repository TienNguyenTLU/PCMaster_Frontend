  ...
  const handleConfirm = async () => {
    ...
    for (const item of po.items) {
      ...
      if (sp <= ip) {
        ...
        toast.error(
          `Giá bán của "${prodName}" phải cao hơn giá nhập (${new Intl.NumberFormat("vi-VN").format(ip)}đ)`,
        );
        return;
      }
    }
      ...
      <div className="bg-white rounded-[16px] shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col my-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0]">
          <h3 className="font-semibold text-[18px] text-[#0f172a]">
            Xác nhận nhận hàng & Cập nhật giá bán
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <p className="text-[14px] text-[#64748b] mb-6">
            Vui lòng nhập giá bán mới cho các sản phẩm trong phiếu nhập này. Giá
            này sẽ được cập nhật trực tiếp vào cửa hàng.
          </p>
          ...
          <div className="flex flex-col gap-4">
            {po.items.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="flex items-center gap-4 p-4 border border-[#e2e8f0] rounded-[12px] bg-[#f8fafc]"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] text-[#64748b]">
                      Giá nhập:{" "}
                      {new Intl.NumberFormat("vi-VN").format(item.importPrice)}{" "}
                      x {item.quantity}
                    </p>
                  </div>
                  <div className="w-[180px]">
                    <label className="text-[11px] font-bold text-[#94a3b8] uppercase mb-1 block">
                      Giá bán mới (VND)
                    </label>
                    <input
                      type="text"
                      value={
                        sellingPrices[item.productId]
                          ? formatVND(sellingPrices[item.productId])
                          : ""
                      }
                      onChange={(e) => {
        ...
        <div className="border-t border-[#e2e8f0] px-6 py-4 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-4 py-2 border border-[#e2e8f0] rounded-[8px] text-[14px] text-[#475569] hover:bg-[#f8fafc] cursor-pointer"
          >
            Hủy
          </button>
          <button
