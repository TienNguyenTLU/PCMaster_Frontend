...
export default function PreviewModal({
  ...
  const handleDownload = () => {
    if (po.documentUrl) {
      const fullUrl = po.documentUrl.startsWith("http")
        ? po.documentUrl
        : `http://localhost:8080${po.documentUrl}`;
  ...
  const viewerUrl = po.documentUrl
    ? po.documentUrl.startsWith("http")
      ? po.documentUrl
      : `http://localhost:8080${po.documentUrl}`
    : "";
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-[#64748b]">
                Mã phiếu: PO-{String(po.id).padStart(4, "0")}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="p-2 hover:bg-[#f1f5f9] rounded-full transition-colors cursor-pointer text-[#64748b]"
            >
              <X className="size-6" />
            </button>
          </div>
          ...
          {po.documentUrl ? (
            <div className="flex-1 w-full h-full relative bg-white flex flex-col items-center justify-center gap-4 p-12">
              ...
              <p className="text-[13px] text-[#64748b]">
                Nhấn nút bên dưới để tải xuống và xem trong Excel/Google Sheets
              </p>
              <a
                href={viewerUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-[#0058be] text-white px-6 py-3 rounded-xl shadow-lg text-[14px] font-bold hover:bg-[#0047a3] transition-all flex items-center gap-2"
              >
            ...
            <div className="p-12 overflow-y-auto flex justify-center flex-1">
              <div className="bg-white w-full max-w-[210mm] min-h-[297mm] shadow-[0_0_40px_rgba(0,0,0,0.05)] p-[20mm] rounded-sm flex flex-col print:shadow-none print:p-0">
                <div className="flex justify-between items-start mb-12">
                  <div>
                    ...
                    <p className="text-[#64748b] font-medium">
                      Số phiếu: <span className="text-[#0f172a]">#{po.id}</span>
                    </p>
                    <p className="text-[#64748b] font-medium">
                      Ngày lập:{" "}
                      <span className="text-[#0f172a]">{createdDate}</span>
                    </p>
                  </div>
                  <div className="text-right">
                    <div
                      className={`px-4 py-1.5 rounded-full text-[13px] font-bold inline-block ${po.status === "RECEIVED" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                    >
                      {po.status === "RECEIVED"
                        ? "ĐÃ NHẬN HÀNG"
                        : "CHỜ NHẬN HÀNG"}
                ...
                <div className="grid grid-cols-2 gap-12 mb-12 border-y border-[#f1f5f9] py-8">
                  <div>
                    <h4 className="text-[12px] font-black text-[#94a3b8] uppercase tracking-[1px] mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[14px] text-[#475569]">
                        Hệ thống build PC & Linh kiện
                      </p>
                      <p className="text-[14px] text-[#475569]">
                        pcmaster.support@gmail.com
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="border-b-2 border-[#0f172a]">
                        <th className="py-4 font-bold text-[#0f172a] text-[14px]">
                          STT
                        </th>
                        <th className="py-4 font-bold text-[#0f172a] text-[14px]">
                          Sản phẩm
                        </th>
                        <th className="py-4 text-right font-bold text-[#0f172a] text-[14px]">
                          Số lượng
                        </th>
                        <th className="py-4 text-right font-bold text-[#0f172a] text-[14px]">
                          Giá nhập
                    ...
                    <tbody className="divide-y divide-[#f1f5f9]">
                      {po.items.map((item, index) => {
                        ...
                        return (
                          <tr key={item.id}>
                            <td className="py-4 text-[#475569] font-medium">
                              {index + 1}
                            </td>
                            <td className="py-4">
                            ...
                            <td className="py-4 text-right text-[#0f172a] font-medium">
                              {item.quantity}
                            </td>
                            <td className="py-4 text-right text-[#0f172a] font-medium">
                            ...
                            <td className="py-4 text-right text-[#0f172a] font-bold">
                              {new Intl.NumberFormat("vi-VN").format(
                ...
                <div className="mt-12 border-t-2 border-[#0f172a] pt-6">
                  <div className="flex justify-end gap-12">
                    <div className="text-right">
                      <p className="text-[14px] font-bold text-[#94a3b8] uppercase mb-2">
                        Tổng số mặt hàng
                      </p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[32px] font-black text-[#0058be]">
                ...
                <div className="mt-[40mm] grid grid-cols-2 gap-12 text-center">
                  <div>
                    <p className="font-bold text-[#0f172a] mb-20 uppercase text-[12px] tracking-[1px]">
                      Nhà cung cấp
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-[#94a3b8] text-[13px] font-bold">
