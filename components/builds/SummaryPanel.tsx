...
import { Cpu, Layers, HardDrive, Tv, Zap, Box, Wind, Monitor, Folder, Fan, ShoppingCart, Loader2, AlertTriangle } from 'lucide-react';
...
export default function SummaryPanel({
  ...
  const currentPsuWattage = Number(psuSpecs.wattage) || Number(psuSpecs.watt) || 0;
  const recommendedWattageVal = aiPsuWattage || (totalTdp > 0 ? Math.ceil((totalTdp + 150) / 50) * 50 : 0);
  const showPsuRecommendation = loadingPsu || (recommendedWattageVal > 0 && (!build.psu || currentPsuWattage < recommendedWattageVal));
