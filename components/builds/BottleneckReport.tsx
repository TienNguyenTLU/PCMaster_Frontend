    ...
    <div className="bg-white border border-[#e8ecf2] rounded-[24px] p-6 flex flex-col gap-5 shadow-sm mb-3">
      ...
      {(!build.cpu || !build.vga || !build.ram) && !loadingBottleneck && (
        <div className="text-[12.5px] text-[#64748b] font-medium py-2 flex items-center gap-2">
          ...
          Hãy chọn đầy đủ <strong>Vi xử lý (CPU)</strong>,{" "}
          <strong>Card đồ họa (GPU)</strong> và <strong>RAM</strong> để hệ thống
          AI tự động phân tích và đưa ra báo cáo tương quan hiệu năng.
        </div>
      )}
        ...
        <div className="bg-rose-50 border border-rose-100 rounded-[16px] p-4 text-[12.5px] text-rose-700 font-semibold flex items-start gap-2.5">
          ...
          <div>
            <p className="font-bold mb-1">
              Không thể phân tích cấu hình hiện tại
            </p>
            <p className="text-[11.5px] text-rose-600 font-medium leading-relaxed">
              {bottleneckError}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {bottleneckResult.map((res: any) => {
              ...
              const colors: Record<
                number,
                { text: string; bg: string; border: string; stroke: string }
              > = {
                0: {
                  text: "text-[#10b981]",
                  bg: "bg-[#10b981]/10",
              ...
              return (
                ...
                >
                  <div className="mb-2">
                    <p className="text-[15px] font-black text-[#0f172a] tracking-tight">
                      {res.resolution}p
                    </p>
                  ...
                  <span
                    className={`text-[11px] font-extrabold px-3 py-1 rounded-[20px] ${currentStyle.bg} ${currentStyle.text} ${currentStyle.border} border uppercase tracking-[0.5px]`}
                  >
                    {currentLabel}
                  </span>
                </div>
