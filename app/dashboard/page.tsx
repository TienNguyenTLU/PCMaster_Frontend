    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        {statCards.map((stat, i) => (
          <div
            key={i}
            className="bg-white border border-[#e2e8f0] rounded-[12px] p-5 flex flex-col gap-2 shadow-sm hover:shadow-md transition-shadow"
          >
            ...
            {chartData.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-[#94a3b8]">
                ...
                <span className="text-[13px]">
                  Chưa có dữ liệu thống kê cho chu kỳ này
                </span>
              </div>
            ) : (
            ...
            {!stats?.recentActivities || stats.recentActivities.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                {stats.recentActivities.map((activity, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-3 border-b border-[#f1f5f9] pb-3 last:border-0 last:pb-0"
                  >
