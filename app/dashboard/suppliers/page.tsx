                        ...
                        {supplier.brandIds && supplier.brandIds.length > 0 ? (
                          supplier.brandIds.map((bid) => {
                            const b = allBrands.find(
                              (brand) => Number(brand.id) === Number(bid),
                            );
                            return b ? (
                              <span
                                key={bid}
                                className="px-2 py-0.5 bg-blue-50 text-[#0058be] text-[11px] font-medium rounded-full border border-blue-100"
                              >
                                {b.name}
                              </span>
                            ) : null;
        ...
        {!loading && !error && filteredSuppliers.length > 0 && (
          <div className="px-6 py-4 border-t border-[#e2e8f0] flex items-center justify-between text-[13px] text-[#64748b]">
            <span>
              ...
              {Math.min((page + 1) * pageSize, filteredSuppliers.length)} trong
              tổng số {filteredSuppliers.length} mục
            </span>
            ...
            <div className="grid grid-cols-3 gap-2">
              {allBrands.map((brand) => (
                <button
                  ...
                  className={`flex items-center gap-2 p-2 rounded-[8px] border text-[13px] transition-all cursor-pointer ${
                    formData.brandIds.includes(Number(brand.id))
