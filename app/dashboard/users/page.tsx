...
import { Plus, Search, Filter, Loader2, X, Lock, Unlock, UserCheck, UserX } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const handleToggleActive = async (user: UserResponse) => {
    ...
    if (!confirm(`Bạn có chắc chắn muốn ${actionText.toLowerCase()} tài khoản "${user.username}"?`)) {
      return;
    }
    ...
    try {
      ...
      const msg = err.response?.data?.message || `Thao tác ${actionText.toLowerCase()} tài khoản thất bại.`;
      toast.error(msg);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[#0f172a] text-[24px] font-semibold tracking-[-0.5px]" style={{ fontFamily: 'Inter, sans-serif' }}>
            Quản lý người dùng
          </h2>
          <p className="text-[#64748b] text-[14px] mt-1" style={{ fontFamily: 'Inter, sans-serif' }}>
            Quản lý tài khoản khách hàng, nhân viên và tạo tài khoản nhân viên mới.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white p-4 rounded-[12px] border border-[#e2e8f0]">
        ...
        <div className="flex items-center gap-2">
          <label className="text-[13px] font-semibold text-[#64748b] whitespace-nowrap" style={{ fontFamily: 'Inter, sans-serif' }}>Vai trò:</label>
          <select
            value={roleFilter}
            onChange={(e) => {
        ...
        {/* Status Filter */}
        <div className="flex items-center gap-2">
          <label className="text-[13px] font-semibold text-[#64748b] whitespace-nowrap" style={{ fontFamily: 'Inter, sans-serif' }}>Trạng thái:</label>
          <select
            value={statusFilter}
            onChange={(e) => {
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-[12px] overflow-hidden shadow-sm">
        <div className="overflow-x-auto min-h-[300px]">
          {loading ? (
            <div className="flex items-center justify-center h-[300px] text-[#64748b]" style={{ fontFamily: 'Inter, sans-serif' }}>
              <Loader2 className="size-5 animate-spin mr-2" /> Đang tải dữ liệu người dùng...
            </div>
          ) : error ? (
            <div className="flex items-center justify-center h-[300px] text-red-500 font-medium" style={{ fontFamily: 'Inter, sans-serif' }}>
              {error}
            </div>
            ...
            <table className="w-full text-left text-[14px]">
              <thead className="bg-[#f8fafc] border-b border-[#e2e8f0] text-[#64748b]">
                <tr>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">ID</th>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Người dùng</th>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Thông tin liên hệ</th>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Vai trò</th>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Trạng thái</th>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider">Ngày đăng ký</th>
                  <th className="px-6 py-4 font-bold uppercase text-[11px] tracking-wider text-right">Khóa / Mở</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#e2e8f0]">
                {paginatedUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-[#f8fafc] transition-colors">
                    <td className="px-6 py-4 font-medium text-[#0f172a]">{user.id}</td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-[#0f172a]">{user.username}</div>
                      <div className="text-[12px] text-[#94a3b8]">{user.email}</div>
                    </td>
                    <td className="px-6 py-4 text-[#475569]">
                      <div className="text-[13px]">{user.phone || <span className="text-[#cbd5e1] italic">Chưa cập nhật</span>}</div>
                      <div className="text-[12px] text-[#94a3b8] truncate max-w-[200px]" title={user.address || ''}>
                        {user.address || <span className="text-[#cbd5e1] italic">Chưa cập nhật</span>}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {user.role === 'ADMIN' ? (
                        <span className="px-2.5 py-1 bg-rose-50 text-rose-600 text-[11px] font-bold rounded-full border border-rose-100 uppercase tracking-[0.5px]">
                          Admin
                        </span>
                      ) : user.role === 'STAFF' ? (
                        <span className="px-2.5 py-1 bg-indigo-50 text-indigo-600 text-[11px] font-bold rounded-full border border-indigo-100 uppercase tracking-[0.5px]">
                          Nhân viên
                        </span>
                      ) : (
                        <span className="px-2.5 py-1 bg-slate-100 text-slate-600 text-[11px] font-bold rounded-full border border-slate-200 uppercase tracking-[0.5px]">
                          Khách hàng
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      {user.role === 'ADMIN' ? (
                        <span className="text-[#cbd5e1] text-[12px] italic">Mặc định</span>
                      ) : (
                        <button
                        ...
                        >
                          {user.active ? <Lock className="size-4" /> : <Unlock className="size-4" />}
                        </button>
                      )}
                    </td>
                  </tr>
        ...
        {!loading && !error && filteredUsers.length > 0 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-[#e2e8f0] bg-[#f8fafc] text-[13px] text-[#64748b]" style={{ fontFamily: 'Inter, sans-serif' }}>
            <span>Hiển thị {page * pageSize + 1} đến {Math.min((page + 1) * pageSize, filteredUsers.length)} trong tổng số {filteredUsers.length} mục</span>
...
function CreateStaffModal({ onClose, onSuccess }: CreateStaffModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Có lỗi xảy ra khi tạo tài khoản nhân viên.';
      toast.error(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-[16px] shadow-2xl w-full max-w-lg flex flex-col my-auto overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#e2e8f0]">
          <h3 className="font-bold text-[18px] text-[#0f172a]" style={{ fontFamily: 'Inter, sans-serif' }}>
        ...
        {/* Modal Body / Form */}
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4 max-h-[85vh] overflow-y-auto" style={{ fontFamily: 'Inter, sans-serif' }}>
          {/* Username */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-bold text-[#475569]">Tên đăng nhập (Username) *</label>
            {errors.username && (
            ...
            <input
              ...
              className={`bg-[#f8fafc] border rounded-[8px] px-4 py-2 text-[14px] focus:outline-none transition-all ${
                errors.username ? 'border-red-500 focus:border-red-500' : 'border-[#e2e8f0] focus:border-[#0058be]'
              }`}
            />
          </div>
          ...
          {/* Email */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-bold text-[#475569]">Email *</label>
            {errors.email && (
              <span className="text-red-500 text-[11px] font-semibold flex items-center gap-1 animate-in fade-in slide-in-from-top-1 duration-150">
                ⚠️ {errors.email}
              </span>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className={`bg-[#f8fafc] border rounded-[8px] px-4 py-2 text-[14px] focus:outline-none transition-all ${
                errors.password ? 'border-red-500 focus:border-red-500' : 'border-[#e2e8f0] focus:border-[#0058be]'
              }`}
            />
          </div>
          ...
          {/* Phone */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-bold text-[#475569]">Số điện thoại</label>
            {errors.phone && (
              <span className="text-red-500 text-[11px] font-semibold flex items-center gap-1 animate-in fade-in slide-in-from-top-1 duration-150">
                ⚠️ {errors.phone}
              </span>
          ...
          {/* Address */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-bold text-[#475569]">Địa chỉ liên hệ</label>
            <input
              type="text"
