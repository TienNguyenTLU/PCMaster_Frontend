...
import { Plus, Search, Edit, Trash2, X, Loader2, Check, Ticket, Calendar, Percent, RefreshCw } from 'lucide-react';
...
export default function CouponsPage() {
  ...
  const getStatusLabel = (c: Coupon) => {
    ...
    if (!c.active) return <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-[11px] font-bold rounded-full border border-gray-200">Đã tắt</span>;
    if (now < start) return <span className="px-2 py-0.5 bg-blue-50 text-blue-600 text-[11px] font-bold rounded-full border border-blue-100">Chưa bắt đầu</span>;
    if (now > end) return <span className="px-2 py-0.5 bg-red-50 text-red-600 text-[11px] font-bold rounded-full border border-red-100">Hết hạn</span>;
    if (c.usageLimit && c.usageCount >= c.usageLimit) return <span className="px-2 py-0.5 bg-amber-50 text-amber-600 text-[11px] font-bold rounded-full border border-amber-100">Hết lượt</span>;
    return <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 text-[11px] font-bold rounded-full border border-emerald-100">Đang hoạt động</span>;
  };
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#64748b] text-[14px] mt-1">Quản lý và tạo mã giảm giá chiết khấu trực tiếp vào hóa đơn của khách hàng.</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto min-h-[300px]">
          {loading ? (
            <div className="flex items-center justify-center h-[300px] text-[#64748b]">
              <Loader2 className="size-5 animate-spin mr-2 text-[#0058be]" /> Đang tải dữ liệu...
            </div>
          ) : error ? (
                  ...
                  <tr key={coupon.id} className="hover:bg-[#f8fafc] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        {coupon.discountType === 'PERCENTAGE' ? <Percent className="size-3.5 text-[#0058be]" /> : <Ticket className="size-3.5 text-emerald-500" />}
                        <span className="font-semibold text-[#0f172a]">{formatDiscount(coupon)}</span>
                      </div>
                    </td>
...
function CouponFormModal({
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: coupon?.startDate ? coupon.startDate.substring(0, 10) : new Date().toISOString().substring(0, 10),
    endDate: coupon?.endDate ? coupon.endDate.substring(0, 10) : new Date(Date.now() + 30 * 86400000).toISOString().substring(0, 10),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.discountValue <= 0) newErrors.discountValue = 'Giá trị giảm giá phải lớn hơn 0.';
    if (formData.discountType === 'PERCENTAGE' && formData.discountValue > 100) newErrors.discountValue = 'Giá trị phần trăm không được vượt quá 100%.';
    ...
    try {
      const payload: CouponRequest = {
        ...
        maxDiscountAmount: formData.discountType === 'PERCENTAGE' && formData.maxDiscountAmount > 0 ? Number(formData.maxDiscountAmount) : null,
      ...
      <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-lg flex flex-col my-auto overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[13px] font-semibold text-[#475569]">Mã code Coupon *</label>
            ...
            {errors.code && <span className="text-red-500 text-[11px] font-medium">⚠️ {errors.code}</span>}
          </div>

          {/* Discount Type */}
          <div className="flex flex-col gap-1">
            <label className="text-[13px] font-semibold text-[#475569]">Hình thức chiết khấu</label>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setFormData(prev => ({ ...prev, discountType: 'PERCENTAGE', discountValue: prev.discountValue > 100 ? 10 : prev.discountValue }))}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              />
              {errors.discountValue && <span className="text-red-500 text-[11px] font-medium">⚠️ {errors.discountValue}</span>}
            </div>

            {/* Max Discount Amount */}
            {formData.discountType === 'PERCENTAGE' && (
              <div className="flex flex-col gap-1">
                <label className="text-[13px] font-semibold text-[#475569]">Giảm tối đa (₫) (0 = Vô hạn)</label>
                <input
                  type="number"
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Min Order Amount */}
            <div className="flex flex-col gap-1">
              <label className="text-[13px] font-semibold text-[#475569]">Đơn tối thiểu áp dụng (₫)</label>
              <input
                type="number"
            ...
            {/* Usage Limit */}
            <div className="flex flex-col gap-1">
              <label className="text-[13px] font-semibold text-[#475569]">Tổng giới hạn lượt dùng (0 = Vô hạn)</label>
              <input
                type="number"
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Start Date */}
            <div className="flex flex-col gap-1">
              <label className="text-[13px] font-semibold text-[#475569]">Ngày bắt đầu hiệu lực</label>
              <input
                type="date"
            ...
            {/* End Date */}
            <div className="flex flex-col gap-1">
              <label className="text-[13px] font-semibold text-[#475569]">Ngày hết hạn</label>
              <input
                type="date"
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            />
            <label htmlFor="activeCheck" className="text-[13px] font-bold text-[#475569] cursor-pointer">Kích hoạt Coupon này</label>
          </div>
