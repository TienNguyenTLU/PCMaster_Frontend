...
export default function PromotionsPage() {
  ...
  const getStatusLabel = (p: Promotion) => {
    ...
    if (!p.active)
      return (
        <span className="px-2.5 py-0.5 bg-gray-100 text-gray-600 text-[11px] font-bold rounded-full border border-gray-200">
          Đã tắt
        </span>
      );
    if (now < start)
    ...
    if (now > end)
      return (
        <span className="px-2.5 py-0.5 bg-red-50 text-red-600 text-[11px] font-bold rounded-full border border-red-100">
          Đã kết thúc
        </span>
      );
    return (
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>
                        ...
                        <strong className="text-[#0f172a]">
                          {p.productIds?.length || 0} sản phẩm
                        </strong>
                      </span>
                    </div>
                  </div>
                  <p className="text-[12px] text-[#94a3b8] line-clamp-2 mt-1">
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await axiosInstance.post<{ url: string }>(
        "/api/admin/media/upload?folder=PCMAster_Storage/Banners",
        uploadData,
        {
          ...
          <div className="w-full lg:w-[420px] p-6 flex flex-col gap-4">
            <div>
              ...
              <p className="text-[#64748b] text-[12px] mt-0.5">
                Lựa chọn các linh kiện máy tính sẽ tham gia chiến dịch này.
              </p>
            </div>
                    ...
                    >
                      <div className="flex flex-col min-w-0 pr-2">
                        ...
                        <span className="text-[11px] text-[#94a3b8] mt-0.5">
                          Giá gốc: {p.price.toLocaleString("vi-VN")}₫
                        </span>
                      </div>
                      <div
                        className={`size-5 shrink-0 rounded-full flex items-center justify-center border ${
                          isChecked
